...
// The Customers tab was removed on 2026-09-08. Customer state is a customer
// concern and now lives entirely on /customers — it was duplicating the
// /customers/[key] record card through a second route with a second
// allow-list, and Delivery -> Configure was the last place anyone would look
// to retire a customer. What is left is genuinely delivery configuration.
type Tab = "roster" | "vocab" | "colours";
      ...
      >
        <div className="px-4 pt-4">
          ...
          <div className="flex gap-4 border-b overflow-x-auto" style={{ borderColor: "var(--brand-metal-line)" }}>
            {(["roster", "vocab", "colours"] as Tab[]).map((t) => (
              <button
                key={t}
                type="button"
                onClick={() => setTab(t)}
              ...
              >
                {t === "roster" ? "Roster" : t === "vocab" ? "Vocabularies" : "Colours"}
              </button>
            ))}
          </div>
        </div>
