...
- `supabase/migrations/` — schema (0001 to 0045). Full dump at `docs/supabase-schema-full.sql` (stale — predates 0020+, regenerate before trusting it).
...
**Customer state is not in Delivery.** Delivery → Configure is Roster, Vocabularies and Colours only — its Customers tab was deleted on 2026-09-08 along with `PATCH /api/customers/roster`, because it duplicated the record card through a second route with a second allow-list and was silently skipping the `CATEGORY_CHANGED` audit event the 360 page logs. One route now: `POST /api/customers/[key]/manual-update`.

**`customers.active` is the single truth for "still a customer".** It was honoured in exactly two places before 2026-09-08 — `customerOptions` in `lib/processes/loader.ts` — so `/customers` listed all 41 customers identically while 24 were retired. The list hides retired behind a toggle now (`CustomerRow.active`); their 360 pages stay reachable. `custom_category` stays a reporting bucket and must not be used to answer "are they a customer": the two disagreed on 6 of 41 rows, and `contradictionFor()` in `customers-browser.tsx` surfaces exactly that — retired-with-live-work, or live work filed under a closed zone. Chip and row badge read through that one function on purpose; two copies of the rule disagreed.

**Customer zones and categories are editable (0045).** `vocabulary_values` gained `rollup` (a category's zone) and `description` (a zone's sub-label), and holds two new *free-text* vocabularies: `customer_zone` and `customer_category`. Unlike the process vocabularies these are **not Postgres enums** — `custom_category` has been plain `text` since 0005 — so adding one is an ordinary INSERT, `add_vocabulary_value()` is not involved, and a value nothing uses can be genuinely **deleted**. `FREE_TEXT_VOCABULARIES` in `lib/vocabulary/store.ts` is the list; `/api/customers/vocabulary` is the route. The group headers on `/customers` (`Focus — the active book`) are `customer_zone` rows, editable label and sub-label; they were `ZONE_ORDER`/`ZONE_DESC` constants before. `zoneForCategory()` takes the vocabulary and falls back to `FALLBACK_ZONES` — a minted category used to fall through `?? "Focus"` and file a wind-down under the active book. **Renaming a category must go through `rename_customer_category()` (0045)**: `custom_category` stores the label, not a slug, so the vocabulary row and every customer carrying it move in one transaction.

Roster management lives in **Delivery → Configure** (Roster and Customers tabs). Roster: rename, roles, **email**, **notes**, **aliases** (view/add/remove — `roster_aliases` had no UI at all before 2026-09-08) and **merge** (`merge_roster_entry()` (0040) and its API route were fully built with no caller, so de-duplication had no front door). Customers: name, category — **which can now mint a new value**, since `custom_category` is free text in Postgres and the closed `<select>` was the only thing making a new category need a code change — plus active/inactive. Marking someone inactive never touches their existing assignments.
